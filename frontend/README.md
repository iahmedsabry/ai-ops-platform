...
- **`docker build -f frontend/Dockerfile -t your-registry/kai-frontend:tag .`** (run from **repository root**) or rely on **GitHub Actions** [`.github/workflows/frontend-image.yml`](../.github/workflows/frontend-image.yml) (Docker Hub; set `DOCKERHUB_USERNAME` / `DOCKERHUB_TOKEN`).
